...
class DistanceEstimationDataset(Dataset):

    """ Dataset with distance of objects """
    ...
    def __getitem__(self, index):
        ...
        bboxes = -1 * np.ones((self.n_bbox, 6))
    ...
    def get_bbox_from_txt_file(self, txt_file):
        ...
        with open(txt_file, 'r') as f:
            for line in f:
                ...
                w = float(vec[3])
                h = float(vec[4])
                d = float(vec[5]) / self.args.dist_norm

                # validating bbox coordinates in range [0.0, 1.0]:
                xmax = x0 + w / 2
                xmin = x0 - w / 2
                ...
                if xmin <= 0 or xmax >= 1:
                    w = min([2 * x0, 2 - 2*x0]) - 2 / self.args.img_size[1]

                if ymin <= 0 or ymax >= 1:
                    h = min([2 * y0, 2 - 2*y0]) - 2 / self.args.img_size[0]

                bbox.append([x0, y0, w, h, d, cls])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="distance database data loader", fromfile_prefix_chars="@")
    parser.convert_arg_line_to_args = CustomArgumentParser(parser).convert_arg_line_to_args

    parser.add_argument('--train-data', type=str,
                        required=True,
                        help='txt file contain all train images path')
    ...
    parser.add_argument('--dist-norm', type=float, default=30,
                        help='image size')

    args, unknow_args = parser.parse_known_args()

    # creating train dataset
    train_set = DistanceEstimationDataset(args, split='train')
